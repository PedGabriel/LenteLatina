...
function createDescriptor(filename, source, { root, isProduction, sourceMap, compiler, template, features }, hmr = false) {
  ...
  const normalizedPath = normalizePath(import_node_path.default.relative(root, filename));
...
function resolveTemplateCompilerOptions(descriptor, options, filename, ssr) {
  ...
  if (transformAssetUrls === false) {
    ...
    if (filename.startsWith(options.root)) {
      ...
      assetUrlOptions = {
        base: (options.devServer.config.server?.origin ?? "") + devBase + slash(import_node_path.default.relative(options.root, import_node_path.default.dirname(filename))),
...
async function transformMain(code, filename, options, pluginContext, ssr, customElement) {
  ...
  if (ssr) {
    const normalizedFilename = normalizePath(import_node_path.default.relative(options.root, filename));
  ...
  if (lang && /tsx?$/.test(lang) && !descriptor.script?.src) {
    const { transformWithOxc } = await import("./node-JQJCCWMH.js");
    if (transformWithOxc) {
...
export {
  ...
  vuePluginCjs as "module.exports",
  parseVueRequest
};
